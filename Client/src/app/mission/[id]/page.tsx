...
const ORG_DESCRIPTIONS: Record<string, string> = {
  ...
  SpaceX: 'Space Exploration Technologies Corp. — American spacecraft manufacturer, launcher, and satellite communications company founded by Elon Musk.',
...
const STATUS_CONFIG: Record<string, { color: string; bg: string; label: string }> = {
  ongoing: { color: 'text-emerald-400', bg: 'bg-emerald-500/20 border-emerald-500/50', label: '● ONGOING' },
  upcoming: { color: 'text-amber-400', bg: 'bg-amber-500/20 border-amber-500/50', label: '◎ UPCOMING' },
  planned: { color: 'text-blue-400', bg: 'bg-blue-500/20 border-blue-500/50', label: '○ PLANNED' },
  completed: { color: 'text-gray-400', bg: 'bg-gray-500/20 border-gray-500/50', label: '✓ COMPLETED' },
};

// Mission-specific data enrichment  
function getMissionDetails(mission: Mission) {
  const daysUntilLaunch = Math.max(0, Math.ceil((new Date(mission.launchDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24)));
...
export default function MissionPage() {
  ...
  const orgGradient = ORG_COLORS[mission.organization] || 'from-violet-500 to-purple-600';
  const orgDesc = ORG_DESCRIPTIONS[mission.organization] || 'International space organization.';
    ...
    <div className="min-h-screen bg-[#0a0a1a] text-white">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0a0a1a]/80 border-b border-white/10">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-1">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${orgGradient} flex items-center justify-center shadow-lg`}>
              <Icons.Rocket className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold">{mission.name}</h1>
              <p className="text-xs text-gray-400">{mission.organization} • {mission.missionType}</p>
            </div>
          </div>
          <span className={`px-3 py-1.5 text-xs rounded-full border font-medium ${statusCfg.bg}`}>
            {statusCfg.label}
          </span>
        </div>
      </header>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="rounded-3xl bg-[#0f0f1f] p-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div className="flex-1">
                <h2 className="text-3xl md:text-4xl font-bold mb-3">{mission.name}</h2>
              ...
              <div className="text-center md:text-right shrink-0">
                ...
                {!isPast && (
                  <p className="text-sm text-gray-500 mt-1">{daysUntilLaunch} days remaining</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Launch Date', value: formatDate(mission.launchDate), icon: Icons.Calendar, color: 'text-violet-400' },
            { label: 'Destination', value: mission.destination, icon: Icons.Target, color: 'text-cyan-400' },
            { label: 'Mission Type', value: mission.missionType, icon: Icons.Layers, color: 'text-emerald-400' },
            { label: 'Crew', value: mission.crew > 0 ? `${mission.crew} astronauts` : 'Uncrewed', icon: Icons.Users, color: 'text-amber-400' },
          ].map((stat, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <h3 className="text-lg font-semibold mb-5 flex items-center gap-2">
              <Icons.Activity className="w-5 h-5 text-cyan-400" /> Mission Progress
            </h3>
            <div className="space-y-3">
              ...
              <div className="h-4 bg-white/10 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: `${mission.progress}%` }} transition={{ duration: 1.5, ease: 'easeOut' }}
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl bg-white/5 text-center">
                ...
                <p className="text-xl font-bold text-white">{isPast ? '—' : daysUntilLaunch}</p>
              </div>
              <div className="p-3 rounded-xl bg-white/5 text-center">
                <Icons.Zap className="w-4 h-4 mx-auto mb-1 text-emerald-400" />
                <p className="text-xs text-gray-500">Status</p>
                <p className={`text-sm font-bold ${statusCfg.color}`}>{mission.status.charAt(0).toUpperCase() + mission.status.slice(1)}</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            className="p-6 rounded-2xl bg-white/5 border border-white/10">
            <h3 className="text-lg font-semibold mb-5 flex items-center gap-2">
              <Icons.Clock className="w-5 h-5 text-violet-400" /> Mission Timeline
            </h3>
            <div className="space-y-0">
              {milestones.map((milestone, i) => {
                const isActive = milestone.complete && (i === milestones.length - 1 || !milestones[i + 1].complete);
                return (
                  <div key={milestone.label} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      <motion.div
                        ...
                        className={`w-4 h-4 rounded-full border-2 shrink-0 ${isActive ? `bg-gradient-to-r ${orgGradient} border-transparent shadow-lg shadow-violet-500/30` :
                            milestone.complete ? 'bg-emerald-500 border-emerald-400' :
                              'bg-transparent border-gray-600'
                          }`}
                      />
                      {i < milestones.length - 1 && (
                        <div className={`w-0.5 h-6 ${milestone.complete ? 'bg-emerald-500/50' : 'bg-gray-700'}`} />
                      )}
                    </div>
                    <span className={`text-sm pb-3 ${isActive ? 'text-white font-semibold' :
                        milestone.complete ? 'text-emerald-400' :
                          'text-gray-500'
                      }`}>
                      {milestone.label}
                      {isActive && <span className="ml-2 text-xs text-cyan-400 animate-pulse">← Current</span>}
                    </span>
                  </div>
                );
              })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${orgGradient} flex items-center justify-center`}>
            ...
            <div className="space-y-2 text-sm text-gray-400">
              ...
              <div className="flex justify-between py-2 border-b border-white/5">
                <span>Crew Size</span>
                <span className="text-white font-medium">{mission.crew > 0 ? `${mission.crew} personnel` : 'Uncrewed'}</span>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          ...
          <div className="flex items-start gap-5">
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${orgGradient} flex items-center justify-center shrink-0 shadow-lg`}>
              <span className="text-2xl font-bold text-white">{mission.organization.charAt(0)}</span>
            </div>
            <div>
