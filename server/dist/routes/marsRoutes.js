...
router.get('/rover/curiosity', async (req, res) => {
    try {
        ...
        const data = await (0, redis_1.withCache)(`mars:curiosity:${solParam}:${pageParam}`, redis_1.CACHE_TTL.MARS_PHOTOS, async () => {
            const response = await axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=${solParam}&page=${pageParam}&api_key=${NASA_API_KEY}`);
...
router.get('/rover/perseverance', async (req, res) => {
    try {
        ...
        const pageParam = page || '1';
        const data = await (0, redis_1.withCache)(`mars:perseverance:${solParam}:${pageParam}`, redis_1.CACHE_TTL.MARS_PHOTOS, async () => {
            const response = await axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/perseverance/photos?sol=${solParam}&page=${pageParam}&api_key=${NASA_API_KEY}`);
            return response.data;
        });
        res.json(data);
    }
...
router.get('/rover/:roverName/manifest', async (req, res) => {
    try {
        ...
        const data = await (0, redis_1.withCache)(`mars:manifest:${roverName}`, redis_1.CACHE_TTL.MARS_DASHBOARD, async () => {
            const response = await axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/manifests/${roverName}?api_key=${NASA_API_KEY}`);
            return response.data;
        });
        res.json(data);
    }
...
router.get('/weather', async (_req, res) => {
    try {
        const data = await (0, redis_1.withCache)('mars:weather', redis_1.CACHE_TTL.MARS_WEATHER, async () => {
            // InSight weather API (may return limited data since InSight ended in Dec 2022)
            let insightData = null;
            try {
                const resp = await axios_1.default.get(`https://api.nasa.gov/insight_weather/?api_key=${NASA_API_KEY}&feedtype=json&ver=1.0`);
...
router.get('/dashboard', async (_req, res) => {
    try {
        const data = await (0, redis_1.withCache)('mars:dashboard', redis_1.CACHE_TTL.MARS_DASHBOARD, async () => {
            const [curiosityManifest, perseveranceManifest] = await Promise.all([
                axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/manifests/curiosity?api_key=${NASA_API_KEY}`).then(r => r.data),
                axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/manifests/perseverance?api_key=${NASA_API_KEY}`).then(r => r.data),
            ]);
            ...
            const [curiosityPhotos, perseverancePhotos] = await Promise.all([
                axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=${cMaxSol}&page=1&api_key=${NASA_API_KEY}`).then(r => r.data).catch(() => ({ photos: [] })),
                axios_1.default.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/perseverance/photos?sol=${pMaxSol}&page=1&api_key=${NASA_API_KEY}`).then(r => r.data).catch(() => ({ photos: [] })),
            ]);
            return {
                rovers: [
                    {
