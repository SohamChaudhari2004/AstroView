...
router.get('/', async (req, res) => {
  try {
    const missions = await Mission.find().sort({ launchDate: 1 });
    res.json(missions);
  } catch (error) {
...
router.get('/status/:status', async (req, res) => {
  try {
    ...
    const missions = await Mission.find({ status }).sort({ launchDate: 1 });
    res.json(missions);
  } catch (error) {
...
router.get('/organizations', async (req, res) => {
  try {
    const organizations = await Mission.distinct('organization');
