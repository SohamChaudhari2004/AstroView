...
export const fetchHazardousAsteroids = async () => {
  try {
    ...
    const response = await axios.get(NEO_WS_URL, {
      params: {
        start_date: today,
        end_date: nextWeek,
    ...
    await redisClient.set(cacheKey, 'fetched', { EX: 3600 }); 
...
export const fetchSolarWeather = async () => {
    try {
        const startDate = new Date().toISOString().split('T')[0];
        console.log(`Fetching Solar Weather for ${startDate}`);

        const response = await axios.get(DONKI_URL, {
            params: {
                startDate: startDate,
                api_key: NASA_API_KEY
            }
        ...
        for (const storm of solarData) {
            ...
            for (const kp of storm.allKpIndex) {
                 ...
                 await SolarStorm.findOneAndUpdate(
                     { gstID: gstID },
                     {
                         startTime: startTime,
                         kpIndex: kp.kpIndex,
